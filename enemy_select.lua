...
enemy_select.load = function()
    -- Load any resources needed for the enemy_select here
end

enemy_select.update = function(dt)
    -- Update the enemy_select local_state here, if needed
end
...
local draw_active_buttons = function()
    ...
    for _, button in ipairs(local_state.active_buttons) do
        ...
        elseif button.hovered then
            love.graphics.setColor(enemy_select.selected_background_color) -- Highlight color
        else
        ...
        love.graphics.setColor(enemy_select.text_color)
...
enemy_select.mousepressed = function(x, y, button, istouch, presses)
...
enemy_select.mousemoved = function(x, y, dx, dy, istouch)
    -- Iterate through the bounds of each character head
    local hovered = false
    for character, headBounds in pairs(bounds) do
...
enemy_select.mousereleased = function(x, y, button, istouch, presses)
    -- Handle mouse release events here
    mousereleased_active_buttons(x, y, button, istouch, presses)
end

enemy_select.keypressed = function(key, scancode, isrepeat)
...
enemy_select.resize = function()
    -- Handle resizing of the enemy_select here
end
